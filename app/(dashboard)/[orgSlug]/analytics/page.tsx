...
export default function AnalyticsDashboard() {
  ...
  useEffect(() => {
    if (orgSlug && selectedBranchId) {
      loadStats();
    }
  }, [selectedBranchId, orgSlug]);

  const loadInitialData = async () => {
    try {
      const res = await fetch(`/api/organizations/${orgSlug}/branches`);
  ...
  const loadStats = async () => {
    setIsLoading(true);
    try {
      const res = await fetch(`/api/organizations/${orgSlug}/stats?branchId=${selectedBranchId}`);
      const data = await res.json();
      setStats(data);
    } catch (e) {
      console.error(e);
         ...
         <Card className="bg-primary border-none rounded-[2.5rem] p-8 shadow-2xl shadow-primary/20 text-white relative overflow-hidden">
            ...
            <div className="relative z-10 space-y-6">
               ...
               <p className="text-sm font-medium opacity-80 leading-relaxed italic">
                 &quot;Based on the current high-intent {stats?.hotLeads} leads from {selectedBranchId === 'all' ? 'global' : branches.find(b => b.id === selectedBranchId)?.city} branches, the system expects a 15% revenue uptick in the next 14 cycles.&quot;
               </p>
