    ...
    <div className="space-y-8 pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-2 border-b border-border/50 dark:border-white/5">
        <div className="flex items-center gap-6">
          ...
          <div>
            <h2 className="text-4xl font-semibold tracking-tight tracking-tighter  lowercase text-slate-900 dark:text-white">/connections</h2>
            <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-[0.3em] mt-1">Sovereign Integration Nexus</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <Badge variant="outline" className="bg-emerald-500/5 text-emerald-500 border-emerald-500/20 text-[10px] font-semibold tracking-tight uppercase tracking-widest px-3 py-1">
            <Zap className="h-3 w-3 mr-1.5 fill-current" /> All Nodes Active
          </Badge>
          <Button variant="outline" className="h-10 rounded-xl px-4 text-[10px] font-semibold tracking-tight uppercase tracking-widest gap-2 bg-white dark:bg-white/5 border-border/50 dark:border-white/10">
      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-2 no-scrollbar">
        {CATEGORIES.map(cat => (
          <button 
            key={cat.id}
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white dark:bg-white/5 border border-border/50 dark:border-white/10 hover:border-indigo-500/30 transition-all group"
          >
            <cat.icon className={cn("h-4 w-4", cat.color)} />
            <span className="text-[10px] font-semibold tracking-tight uppercase tracking-widest text-slate-600 dark:text-slate-300 group-hover:text-indigo-500 transition-colors uppercase">{cat.label}</span>
          </button>
        ))}
      </div>
        ...
        {INTEGRATION_MARKETPLACE.map(item => {
          ...
          return (
            <Card 
              key={item.id}
              className={cn(
                "relative overflow-hidden border-none rounded-xl p-8 shadow-sm transition-all duration-500 group",
            ...
            >
              ...
              <div className="absolute top-6 right-8">
                {item.isComingSoon ? (
                    <Badge className="bg-slate-200 text-slate-500 text-[8px] font-semibold tracking-tight uppercase px-2 h-5 border-none">Coming Soon</Badge>
                ) : isConfigured ? (
                    <div className="flex items-center gap-2">
                         <div className={cn("h-1.5 w-1.5 rounded-full", isEnabled ? "bg-emerald-500 animate-pulse" : "bg-slate-300")} />
                         <span className={cn("text-[9px] font-semibold tracking-tight uppercase tracking-widest", isEnabled ? "text-emerald-500" : "text-slate-400")}>
                            {isEnabled ? 'Live' : 'Paused'}
                         </span>
                    </div>
                ) : (
                    <Badge variant="outline" className="text-[8px] font-semibold tracking-tight uppercase opacity-40">Not Configured</Badge>
                )}
              </div>

              <div className="space-y-6">
                <div className="flex items-center gap-5">
                  <div className="h-14 w-14 rounded-xl bg-white dark:bg-slate-800 flex items-center justify-center shadow-sm ring-1 ring-slate-100 dark:ring-white/10 group-hover:scale-110 transition-transform duration-500">
                    <IntegrationIcon slug={item.iconSlug} size={32} />
                  </div>
                  <div>
                    <h3 className="text-lg font-semibold tracking-tight text-slate-900 dark:text-white leading-tight">{item.name}</h3>
                    <p className="text-[9px] font-semibold tracking-tight uppercase tracking-widest text-indigo-500/60 mt-0.5">{item.category}</p>
                  </div>
                </div>
                ...
                <div className="pt-4 flex items-center gap-3">
                  <Button 
                    ...
                    className={cn(
                        "flex-1 h-11 rounded-xl text-[10px] font-semibold tracking-tight uppercase tracking-widest transition-all",
                        isConfigured ? "bg-slate-900 dark:bg-primary text-white" : "bg-white dark:bg-white/5 text-slate-900 dark:text-white border border-border/50 dark:border-white/10"
                    )}
                  >
      ...
      {/* SECURITY FOOTER */}
      <div className="max-w-4xl mx-auto p-8 rounded-xl bg-slate-900 dark:bg-black/40 text-white flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-4">
              <div className="h-10 w-10 rounded-xl bg-emerald-500/20 flex items-center justify-center">
                  <ShieldCheck className="h-5 w-5 text-emerald-400" />
              </div>
              <div>
                  <h4 className="text-sm font-semibold tracking-tight uppercase tracking-tight">Clinical Grade Security</h4>
                  <p className="text-[10px] text-slate-400 font-medium">All credentials are encrypted and stored in your private AWS instance.</p>
              </div>
          </div>
          <Button variant="outline" className="border-white/10 text-white hover:bg-white/10 text-[9px] font-semibold tracking-tight uppercase tracking-widest h-10 px-6 rounded-xl">
