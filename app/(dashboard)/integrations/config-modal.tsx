...
export function ConfigModal({ isOpen, onClose, integrationId, existingData, onSuccess }: ConfigModalProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[500px] rounded-[2.5rem] bg-white/95 dark:bg-slate-900/95 backdrop-blur-2xl border-none shadow-2xl p-8 overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-br from-indigo-500/5 via-transparent to-transparent pointer-events-none" />
        
        <DialogHeader className="relative space-y-4">
          <div className="flex items-center gap-4">
            <div className="h-14 w-14 rounded-2xl bg-white dark:bg-slate-800 flex items-center justify-center shadow-sm ring-1 ring-slate-100 dark:ring-white/10">
          ...
          {fields.length > 0 ? (
            fields.map(field => (
              <div key={field.key} className="space-y-2">
                <Label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 pl-1">{field.label}</Label>
                <Input 
                  value={formData[field.key] || ""}
                  onChange={(e) => setFormData(prev => ({ ...prev, [field.key]: e.target.value }))}
                  placeholder={field.placeholder}
                  className="h-12 rounded-xl bg-slate-50 dark:bg-white/5 border-slate-200 dark:border-white/10 focus-visible:ring-indigo-500/20 font-medium"
                />
              </div>
            ))
          ) : (
            <div className="py-12 text-center space-y-3 opacity-40">
                ...
                <p className="text-xs font-black uppercase tracking-widest">Default Parameters Active</p>
        ...
        <DialogFooter className="relative gap-3">
          <Button variant="ghost" onClick={onClose} className="rounded-xl h-12 px-6 text-[10px] font-black uppercase tracking-widest hover:bg-slate-100">
            Abort
          </Button>
          <Button 
            onClick={handleSave}
            disabled={loading}
            className="flex-1 bg-slate-900 dark:bg-primary text-white h-12 rounded-xl text-[10px] font-black uppercase tracking-widest gap-2 shadow-lg hover:shadow-primary/20 active:scale-95 transition-all"
          >
