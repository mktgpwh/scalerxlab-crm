...
export default async function DashboardHome() {
  ...
  const quickLinks = [
    { title: "Intelligence Hub", desc: "Executive Analytics Matrix", url: "/leads", icon: Users, color: "text-indigo-500", bg: "bg-indigo-500/10", badge: `${totalLeads} Leads` },
    { title: "Capture Pipeline", desc: "Medical Funnel Kanban Board", url: "/pipeline", icon: LayoutDashboard, color: "text-emerald-500", bg: "bg-emerald-500/10", badge: `${hotLeads} Hot` },
    { title: "Shared Inbox", desc: "WhatsApp · FB · Instagram", url: "/inbox", icon: WhatsAppIcon, color: "text-[#25D366]", bg: "bg-[#25D366]/10", badge: "Live" },
    { title: "Analytics Hub", desc: "Regional & Performance Data", url: "/analytics", icon: BarChart3, color: "text-blue-500", bg: "bg-blue-500/10", badge: null },
    { title: "Integrations", desc: "Ad Channels & Webhooks", url: "/integrations", icon: Command, color: "text-slate-500", bg: "bg-slate-100", badge: null },
    { title: "Activity Logs", desc: "DPDPA Sovereign Ledger", url: "/activity", icon: Activity, color: "text-purple-500", bg: "bg-purple-500/10", badge: null },
  ];
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-2 border-b border-slate-100 dark:border-white/5">
        <div className="flex items-center gap-6">
          <div className="h-20 w-auto">
            <Image src="/scalerxlab-logo.png" alt={clinicName} width={180} height={45} className="object-contain" />
          </div>
          <div className="h-12 w-px bg-slate-200 dark:bg-white/10 hidden md:block" />
          <div>
            <p className="text-[10px] font-black uppercase tracking-[0.3em] text-primary mb-1">{clinicName} Hub Active</p>
            <h2 className="text-4xl font-black tracking-tighter italic lowercase text-slate-900 dark:text-white">
              /{clinicName.toLowerCase().replace(/\s+/g, "")}.hub
            </h2>
            <p className="text-sm font-medium text-slate-500 mt-1">
              Welcome back to <span className="font-black text-slate-900 dark:text-white">{clinicName}</span> — All systems nominal.
            </p>
          </div>
        </div>
