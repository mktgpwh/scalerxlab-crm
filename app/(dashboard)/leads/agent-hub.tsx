  ...
  return (
    <div className="space-y-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        {/* Daily Scorecard */}
        <Card className="lg:col-span-1 rounded-[3rem] border-none shadow-sm ring-1 ring-slate-200/50 p-8 bg-gradient-to-br from-white to-slate-50/50 overflow-hidden relative group">
           ...
           <div className="relative z-10 flex flex-col h-full justify-between">
              <div>
                <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-[10px] font-black text-primary uppercase tracking-[0.2em]">Live Performance</span>
                </div>
                <h2 className="text-3xl font-black italic tracking-tighter">Daily Scorecard</h2>
                <p className="text-sm font-medium text-slate-500 mt-2 italic">Operation status for your clinic node.</p>
              </div>

              <div className="grid grid-cols-2 gap-4 mt-8">
                <div className="p-5 rounded-3xl bg-white ring-1 ring-slate-100 shadow-sm">
                    <p className="text-[10px] font-black uppercase text-slate-400 mb-1">Assigned Today</p>
                    <p className="text-3xl font-black tracking-tighter">{stats.leadsToday}</p>
                </div>
                <div className="p-5 rounded-3xl bg-emerald-50 ring-1 ring-emerald-100 shadow-sm">
                    <p className="text-[10px] font-black uppercase text-emerald-600 mb-1">Converted</p>
                    <p className="text-3xl font-black tracking-tighter text-emerald-700">{stats.wonToday}</p>
                </div>
              </div>
              ...
              <Button 
                ...
                className="w-full h-14 rounded-2xl bg-slate-900 hover:bg-black text-white text-[11px] font-black uppercase tracking-[0.2em] mt-6 shadow-xl group/btn transition-all"
              >
           ...
           <div className="relative z-10">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                 {priorityLeads.length > 0 ? priorityLeads.map((l: any) => (
                   <div key={l.id} className="p-5 rounded-3xl bg-white/5 border border-white/10 hover:bg-white/10 transition-all cursor-pointer group">
                      <div className="flex items-center justify-between mb-3">
                        <div className="h-2 w-2 rounded-full bg-primary animate-pulse shadow-[0_0_10px_rgba(99,102,241,0.5)]" />
                        <span className="text-[9px] font-black text-white/40 uppercase bg-white/5 px-2 py-0.5 rounded-full">Score {l.aiScore}%</span>
                      </div>
                      <p className="font-bold text-sm text-white truncate mb-1">{l.name}</p>
                   ...
                   <div className="col-span-3 py-10 flex flex-col items-center justify-center opacity-40">
                      ...
                      <p className="text-[10px] font-black uppercase tracking-widest text-center">No high-intensity signals detected at this clinical node.</p>
                   </div>
                 )}
              </div>
           </div>
      ...
      <div className="space-y-6">
        <div className="flex items-center gap-2">
            ...
            <h3 className="text-xl font-black italic tracking-tight">Personal Conversion Matrix</h3>
        </div>
        <div className="grid grid-cols-1 gap-10">
            <section className="space-y-4">
                <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 px-1">Speciality Funnel (Personal)</p>
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
                ...
                <h3 className="text-xl font-black italic tracking-tight">Personal Portfolio</h3>
            </div>
        </div>
        <LeadsDataView 
            leads={myLeads} 
